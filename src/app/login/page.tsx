...
function FormInner() {
  ...
  function goNext(requiresFace?: boolean, isNew?: boolean) {
    const dest =
      isNew && !(searchParams.get("next"))
        ? "/onboarding"
  ...
  const googleHref = `/api/auth/google?mode=signin&next=${encodeURIComponent(next)}`;
    ...
    <div className="container-page flex min-h-[70vh] items-center justify-center py-12">
      <div className="w-full max-w-md rounded-2xl border border-border bg-white p-8 shadow-sm">
        ...
        <p className="mt-2 text-sm text-foreground-muted">
          Sign in with phone OTP — face verification is only for Trusted Seller / high-risk.
        </p>

        <div className="mt-6">
          {mode === "phone" ? (
            <PhoneAuthForm
              onSuccess={({ requiresFaceVerification, isNew }) =>
                goNext(requiresFaceVerification, isNew)
              }
            />
          ) : (
