...
const envSchema = z.object({
  ...
  STORAGE_PROVIDER: z.enum(["local", "mock", "vercel-blob"]).default("local"),
  BLOB_READ_WRITE_TOKEN: z.string().default(""),
  PAYMENT_PROVIDER: z.enum(["mock"]).default("mock"),
  EMAIL_PROVIDER: z.enum(["mock"]).default("mock"),
  SMS_PROVIDER: z.enum(["mock", "smsgate"]).default("mock"),
  SMSGATE_URL: z.string().default("http://192.168.29.93:8080"),
  SMSGATE_USERNAME: z.string().default(""),
  SMSGATE_PASSWORD: z.string().default(""),
  SMSGATE_SIM: z.string().default("1"),
...
function readRawEnv(): Record<string, string | undefined> {
  return {
    ...
    SMSGATE_URL: process.env.SMSGATE_URL,
    SMSGATE_USERNAME: process.env.SMSGATE_USERNAME,
    SMSGATE_PASSWORD: process.env.SMSGATE_PASSWORD,
